...
async function Module(moduleArg = {}) {
	...
	(function() {
		...
		if (currentNodeVersion < 160000) {
			throw new Error(
				`This emscripten-generated code requires node v${packedVersionToHumanReadable(
					160000
				)} (detected v${packedVersionToHumanReadable(currentNodeVersion)})`
			);
		}
		...
		var currentSafariVersion =
			...
			navigator.userAgent.match(/Version\/(\d+\.?\d*\.?\d*)/)
				? humanReadableVersionToPacked(
						navigator.userAgent.match(/Version\/(\d+\.?\d*\.?\d*)/)[1]
				  )
				: TARGET_NOT_SUPPORTED;
		if (currentSafariVersion < 150000) {
			throw new Error(
				`This emscripten-generated code requires Safari v${packedVersionToHumanReadable(
					150000
				)} (detected v${currentSafariVersion})`
			);
		}
...
/** not-@type {!BigUint64Array} */
	...
	function findWasmBinary() {
		if (Module["locateFile"]) {
			return locateFile("cspuz_solver_backend.wasm");
		}

		// Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
		return new URL("cspuz_solver_backend.wasm", import.meta.url).href;
	}
	...
	var _solve_problem = (Module["_solve_problem"] = makeInvalidEarlyAccess(
		"_solve_problem"
	));
	var _enumerate_answers_problem = (Module[
		"_enumerate_answers_problem"
	] = makeInvalidEarlyAccess("_enumerate_answers_problem"));
	...
	var __emscripten_tempret_set = makeInvalidEarlyAccess(
	...
	function assignWasmExports(wasmExports) {
		assert(
			typeof wasmExports["solve_problem"] != "undefined",
			"missing Wasm export: solve_problem"
		);
		_solve_problem = Module["_solve_problem"] = createExportWrapper(
			"solve_problem",
			2
		);
		assert(
			typeof wasmExports["enumerate_answers_problem"] != "undefined",
			"missing Wasm export: enumerate_answers_problem"
		);
		_enumerate_answers_problem = Module[
			"_enumerate_answers_problem"
		] = createExportWrapper("enumerate_answers_problem", 3);
		assert(
		...
		assert(
			typeof wasmExports["htonl"] != "undefined",
			"missing Wasm export: htonl"
		);
		_htonl = createExportWrapper("htonl", 1);
		assert(
			typeof wasmExports["htons"] != "undefined",
			"missing Wasm export: htons"
		);
		_htons = createExportWrapper("htons", 1);
		assert(
			typeof wasmExports["ntohs"] != "undefined",
			"missing Wasm export: ntohs"
		);
		...
		__emscripten_tempret_set = createExportWrapper(
