...
(function(pidlist, classbase) {
	...
	MouseEvent: {
			...
			while (moving) {
				...
				if (this.mousestart && cell.isDestination()) {
					...
					if (
						...
						if (
							border.isLine() ||
							border.sidecell[0].distance > 0 ||
							border.sidecell[1].distance > 0
						) {
		...
		inputqnum_main: function(cell) {
			...
			if (this.inputcell_herugolf(cell)) {
				return;
			}
			if (cell.ques !== 0) {
				return;
		...
		prehook: {
			line: function(num) {
				return (
					...
					!this.sidecell[0].isViaPoint() &&
					!this.sidecell[1].isViaPoint()
				);
			}
		}
	},
	...
	Graphic: {
		...
		getQuesNumberText: function(cell) {
			if (this.puzzle.execConfig("dispmove")) {
				if (!cell.isDestination() || cell.base.qnum < 0) {
					return "";
				}
		...
		getLineColor: function(border) {
			...
			if (border.isLine() || border.isLineBySolver()) {
				...
				if (info === 1) {
					...
					return this.puzzle.execConfig("dispmove")
						? this.movetrialcolor
						: this.trialcolor;
				}
			...
			for (var i = 0; i < clist.length; i++) {
				...
				if (cell.qnum === -1 && !this.puzzle.execConfig("dispmove")) {
					...
					if (
						adb.top.isLine() &&
						(cell.lcnt === 1 ||
							(cell.isViaPoint() && adc.top.distance === cell.distance + 1))
					) {
						dir = 2;
						border = adb.top;
					} else if (
						adb.bottom.isLine() &&
						(cell.lcnt === 1 ||
							(cell.isViaPoint() && adc.bottom.distance === cell.distance + 1))
					) {
						dir = 1;
						border = adb.bottom;
					} else if (
						adb.left.isLine() &&
						(cell.lcnt === 1 ||
							(cell.isViaPoint() && adc.left.distance === cell.distance + 1))
					) {
						dir = 4;
						border = adb.left;
					} else if (
						adb.right.isLine() &&
						(cell.lcnt === 1 ||
							(cell.isViaPoint() && adc.right.distance === cell.distance + 1))
					) {
						dir = 3;
						border = adb.right;
					}
	...
	Encode: {
		...
		encodeNumber16_herugolf: function() {
			...
			for (var c = 0; c < bd.cell.length; c++) {
				...
				if (count === 0) {
					cm += pstr;
				} else if (pstr || count === 18) {
					cm += (17 + count).toString(36) + pstr;
					count = 0;
				}
			}
			if (count > 0) {
				cm += (17 + count).toString(36);
			}
	...
	AnsCheck: {
		...
		checkLineOverHole: function() {
			this.checkAllCell(function(cell) {
				return cell.ques === 31 && cell.lcnt >= 2;
			}, "laOnHole");
		},
		checkStopHalfway: function() {
			this.checkAllCell(function(cell) {
				return cell.lcnt === 1 && cell.distance >= 0 && !cell.isViaPoint();
			}, "laLenNe");
		},
		checkCurveHalfway: function() {
			this.checkAllCell(function(cell) {
				return cell.isLineCurve() && !cell.isViaPoint();
			}, "laCurve");
		},
		checkWaterHazard: function() {
			this.checkAllCell(function(cell) {
				return cell.ques === 6 && cell.isViaPoint();
			}, "laWaterHazard");
		},
		checkCupIn: function() {
			if (this.puzzle.execConfig("dispmove")) {
				...
				this.checkAllCell(function(cell) {
					return cell.qnum !== -1 && cell.getDestination().ques !== 31;
				}, "nmOutOfHole");
			}
		},
