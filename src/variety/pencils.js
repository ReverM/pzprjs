...
(function(pidlist, classbase) {
	...
	Border: {
		getArrow: function() {
			var cell0 = this.sidecell[0],
				cell1 = this.sidecell[1];
			if (this.isvert) {
				if (cell0.getPencilDir() === cell0.LT) {
					return cell0.LT;
				}
				if (cell1.getPencilDir() === cell1.RT) {
					return cell1.RT;
				}
			} else {
				if (cell0.getPencilDir() === cell0.UP) {
					return cell0.UP;
				}
				if (cell1.getPencilDir() === cell1.DN) {
					return cell1.DN;
				}
			}
			return 0;
	...
	AnsCheck: {
		...
		pencils_checkLines: function(func, code) {
			...
			for (var c = 0; c < comps.length; c++) {
				var comp = comps[c];
				var ends = comp.clist.filter(function(cell) {
					return cell.isTip() && cell.lcnt === 1;
				});
				if (func(ends)) {
					this.failcode.add(code);
