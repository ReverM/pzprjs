	...
	MouseEvent: {
		...
		inputLine: function() {
			...
			if (!border.isnull) {
				...
				if (this.prevblist.length === 0 || !this.prevblist.include(border)) {
					this.inputData = null;
				}

				if (this.inputData === null) {
					this.inputData = [1, 2, 0][border.line];
				}
				if (this.inputData > 0 && (dir === border.UP || dir === border.LT)) {
					borders.reverse();
				} // 色分けの都合上の処理
	...
	Cell: {
		...
		getCountOfBridges: function() {
			...
			for (var i = 0; i < cblist.length; i++) {
				...
				if (!border.isnull && border.line > 0) {
					cnt += border.line;
				}
			}
			return cnt;
		...
		isCmp: function() {
			...
			return (
				...
				this.qnum === this.getCountOfBridges()
			);
		},
		...
		getState: function() {
			...
			var uborder = this.adjborder.top,
				lborder = this.adjborder.left;
